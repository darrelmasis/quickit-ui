export const INPUT_GROUP_THEME_CLASSES = {
  light: {
    neutral: {
      frame: "border-neutral-300",
      separator: "bg-neutral-300",
      focus: "focus-within:ring-neutral-400/50",
    },
    primary: {
      frame: "border-blue-300/90",
      separator: "bg-blue-300/90",
      focus: "focus-within:ring-blue-400/45",
    },
    secondary: {
      frame: "border-purple-300/90",
      separator: "bg-purple-300/90",
      focus: "focus-within:ring-purple-400/45",
    },
    success: {
      frame: "border-green-300/90",
      separator: "bg-green-300/90",
      focus: "focus-within:ring-green-400/45",
    },
    danger: {
      frame: "border-red-300/90",
      separator: "bg-red-300/90",
      focus: "focus-within:ring-red-400/45",
    },
    warning: {
      frame: "border-amber-300/90",
      separator: "bg-amber-300/90",
      focus: "focus-within:ring-amber-400/45",
    },
    info: {
      frame: "border-cyan-300/90",
      separator: "bg-cyan-300/90",
      focus: "focus-within:ring-cyan-400/45",
    },
    light: {
      frame: "border-neutral-200",
      separator: "bg-neutral-200",
      focus: "focus-within:ring-neutral-400/45",
    },
    dark: {
      frame: "border-neutral-500",
      separator: "bg-neutral-500",
      focus: "focus-within:ring-neutral-700/32",
    },
  },
  dark: {
    neutral: {
      frame: "border-neutral-700",
      separator: "bg-neutral-700",
      focus: "focus-within:ring-neutral-500/32",
    },
    primary: {
      frame: "border-blue-500/55",
      separator: "bg-blue-500/55",
      focus: "focus-within:ring-blue-400/30",
    },
    secondary: {
      frame: "border-purple-500/55",
      separator: "bg-purple-500/55",
      focus: "focus-within:ring-purple-400/30",
    },
    success: {
      frame: "border-green-500/55",
      separator: "bg-green-500/55",
      focus: "focus-within:ring-green-400/30",
    },
    danger: {
      frame: "border-red-500/55",
      separator: "bg-red-500/55",
      focus: "focus-within:ring-red-400/30",
    },
    warning: {
      frame: "border-amber-500/55",
      separator: "bg-amber-500/55",
      focus: "focus-within:ring-amber-400/30",
    },
    info: {
      frame: "border-cyan-500/55",
      separator: "bg-cyan-500/55",
      focus: "focus-within:ring-cyan-400/30",
    },
    light: {
      frame: "border-neutral-500",
      separator: "bg-neutral-500",
      focus: "focus-within:ring-neutral-400/30",
    },
    dark: {
      frame: "border-neutral-700",
      separator: "bg-neutral-700",
      focus: "focus-within:ring-neutral-600/35",
    },
  },
};
