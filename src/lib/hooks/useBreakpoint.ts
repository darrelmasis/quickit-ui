...
function normalizeBreakpoints(breakpoints: Record<string, number> = {}) {
  return {
    ...DEFAULT_BREAKPOINTS,
    ...breakpoints,
  };
...
function resolveBreakpoint(width: number | null, breakpoints: Record<string, number>) {
  if (width == null) {
    return null;
  }
...
export function useBreakpoint(options: { breakpoints?: Record<string, number> } = {}) {
