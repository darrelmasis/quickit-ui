...
export function isQuickitTokenValue<T>(collection: readonly T[] | T[], value: unknown): value is T {
  return Array.isArray(collection) && (collection as unknown[]).includes(value);
}

export function resolveQuickitToken<T>(collection: readonly T[] | T[], value: unknown, fallback: T): T {
