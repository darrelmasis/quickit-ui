...
export const ACTION_CONTROL_SIZE_CLASSES: Record<string, Record<string, string>> = {
  default: {
...
export function resolveActionTheme(theme: string) {
  return resolveQuickitThemeMode(theme);
}

function prefixStateClasses(prefix: string, classes: string) {
  return classes
    .split(/\s+/)
    .filter(Boolean)
...
export function resolveActionShape(shape: string) {
  return resolveQuickitToken(QUICKIT_BUTTON_SHAPES, shape, "default");
}

export function resolveActionSize(size: string) {
  return resolveQuickitToken(QUICKIT_CONTROL_SIZES, size, "md");
}

export function resolveActionVariant(theme: string, variant: string) {
  ...
  return (ACTION_CONTROL_THEME_CLASSES as Record<string, Record<string, Record<string, string>>>)[resolvedTheme][resolvedVariant]
    ? resolvedVariant
    : "solid";
}
...
export function resolveActionActiveStateClasses(
  ...
  return (
    (ACTION_CONTROL_ACTIVE_THEME_CLASSES as Record<string, Record<string, Record<string, string>>>)[resolvedTheme][resolvedVariant]?.[
      resolvedColor
    ] ?? (ACTION_CONTROL_ACTIVE_THEME_CLASSES as Record<string, Record<string, Record<string, string>>>)[resolvedTheme].solid.primary
  );
}
...
const RIPPLE_HUE_MAP: Record<string, Record<string, string>> = {
...
export function resolveActionRippleStyles(theme: string, variant: string, color: string) {
  const resolvedTheme = resolveActionTheme(theme);
  const resolvedVariant = resolveActionVariant(resolvedTheme, variant);
  const resolvedColor = resolveActionColor(
    resolvedTheme,
...
export function getActionControlRadius(shape: string, size: string) {
