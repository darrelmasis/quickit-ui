...
const modalOverlay = createOverlayStack("modal", 50);

function getModalTransform(isVisible: boolean): string {
  if (isVisible) {
...
const MODAL_PRIMITIVES = {
  ...
  dialog: [
    "qk-overlay-surface pointer-events-auto flex max-h-[calc(100dvh-1rem)] sm:max-h-[calc(100dvh-2rem)] w-full flex-col overflow-hidden",
    "rounded-[var(--qk-radius-3xl)] border transform-gpu will-change-transform",
  ].join(" "),
  header:
...
export function Modal({
  ...
  closeOnEscape = true,
  defaultOpen = false,
  /** If `false`, the backdrop does not capture clicks (non-blocking overlay). */
  ...
  const titleId = `qk-modal-title-${modalId}`;
  const descriptionId = `qk-modal-description-${modalId}`;
  ...
  useEffect(() => {
    ...
    modalOverlay.addToStack(modalId);
    lockAppScroll();
    const nextZIndex = modalOverlay.incrementZIndex(customZIndex);
    const frameId = window.requestAnimationFrame(() => {
    ...
    return () => {
      window.cancelAnimationFrame(frameId);
      modalOverlay.removeFromStack(modalId);
      unlockAppScroll();
      modalOverlay.decrementZIndex(customZIndex);
    };
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key !== "Escape" || !modalOverlay.isTopmost(modalId)) {
        return;
      }
  ...
  const value = useMemo(
    () => ({
      ...
      instanceZIndex,
      isTopmost: () => modalOverlay.isTopmost(modalId),
      maxWidth,
      open,
      outsideClick,
      registerDescription,
  ...
  if (asChild) {
    ...
    return cloneElement(child, {
      ...
      onClick: (event: MouseEvent) => {
        child.props.onClick?.(event as MouseEvent);
...
const ModalContent = forwardRef(function ModalContent({ children, className }: { children: ReactNode; className?: string }, ref) {
  const {
    blockingOverlay,
    close,
...
export function ModalTitle({ centered = true, children, className, id }: { centered?: boolean; children: ReactNode; className?: string; id?: string }) {
