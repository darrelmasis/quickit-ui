...
function getBuiltinKindIcon(kind: string, theme: string) {
...
function stackAnimClasses(position: string, dismissing: boolean) {
...
function stackTransformOrigin(position: string) {
  const verticalOrigin = position.startsWith("top") ? "top" : "bottom";

  return `${verticalOrigin} center`;
}

function clampVisibleToasts(n: number) {
...
function resolveGap(
  gapProp: number | { collapsed?: number; expanded?: number } | null | undefined,
  expanded: boolean,
) {
  if (gapProp == null) {
...
function resolveToastIcon(
  item: ToastItem,
  { defaultIcon, icons, theme }: { defaultIcon?: React.ReactNode; icons?: Record<string, React.ReactNode>; theme: string },
) {
  if (item.icon != null) {
    return item.icon;
  }
...
function getToastAnnouncementProps(item: ToastItem): { role: string; "aria-atomic": string; "aria-live": string } {
  if (item.kind === "error") {
    return {
      role: "alert",
      "aria-atomic": "true",
            ...
            >
              ...
              >
                <div
                  ...
                  className={cn(
                    "qk-toast-surface pointer-events-auto w-full min-w-0 max-w-full rounded-[var(--qk-radius-2xl)] border px-4 py-3",
